...
class SegformerCrossAttentionWrapperV2(nn.Module):
    def __init__(self, segformer_name='nvidia/mit-b5', 
        ...
        # --- Hybrid Branch (PRETRAINED statt random) ---
        base_model_aux = SegformerModel.from_pretrained(segformer_name)
        self.encoder_feat_hybrid = base_model_aux.encoder
        ...
        self.pe_base_size = 32  # z.B. 32x32 als Basis (kannst du anpassen)

        self.pos_embeds = nn.ParameterList([
            nn.Parameter(
                torch.zeros(1, c, self.pe_base_size, self.pe_base_size)
            )
            for c in config.hidden_sizes
        ])

        # Optional: kleine Init (statt alles 0)
    ...
    def forward(self, image_rgb, labels=None, return_attention=False):
        ...
        for i in range(4):
            ...
            # --- Learnable 2D-PE: auf aktuelle Größe bringen ---
            # self.pos_embeds[i]: (1, C, pe_base_size, pe_base_size)
            pe_2d = F.interpolate(
                self.pos_embeds[i], 
                size=(Hs, Ws),
                mode='bilinear',
            ...
            feat_flat = feat_small.flatten(2).transpose(1, 2)      # (B, N, C)

            # PE auch flatten
            pe_flat = pe_2d.flatten(2).transpose(1, 2)             # (1, N, C)

            # Broadcast über Batch
            rgb_flat  = rgb_flat  + pe_flat
            feat_flat = feat_flat + pe_flat
            ...
            fused = fused.transpose(1, 2).view(B, C, Hs, Ws)
