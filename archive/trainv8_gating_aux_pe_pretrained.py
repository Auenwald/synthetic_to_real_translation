...
from archive.segformer_crossattention_wrapper_pe_aux_pretrained import *
import os
import json
...
def get_optimizer_and_scheduler(model, optimizer_name='adamw', lr=1e-5, total_steps=10000, warmup_steps=500, schedule=None, power=0.9, min_lr=1e-6):
    ...
    param_groups = [
        ...
        {"params": model.encoder_feat_hybrid.parameters(), "lr": base_lr },
...
def main():
    ...
    model = SegformerCrossAttentionWrapperV2(segformer_name='nvidia/mit-b5', mode="edge", num_heads=4)
