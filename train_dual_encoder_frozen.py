...
import model_utils
from segformer_dual_encoder import *
import os
import json
...
def main():
    ...
    model = SegformerDualEncoderWrapper(segformer_name='nvidia/mit-b5', num_heads=4, use_attention=True)
...
def train(train_loader, model, optim, loss_fn, DEVICE, ema, scheduler, PRINT_INTERVAL, AVERAGING_INTERVAL, SOURCE_DATASET_NAME):
    ...
    for i, (data, targets) in enumerate(train_loader):
        ...
        logits, rgb_hidden_states, rgb_frozen_hidden_states = model(data)
        ...
        distill_loss = 0.0
        for F_trainable, F_frozen in zip(rgb_hidden_states, rgb_frozen_hidden_states):
            distill_loss += ((F_trainable - F_frozen.detach()) ** 2).mean()
...
def validate(val_loader, model, DEVICE, LOG_PATH, applied_ema, dataset_name, epoch, max_epochs):
    ...
    for idx, (data, targets) in enumerate(val_loader):
        ...
        with torch.no_grad():
            output, rgb_hidden_states, rgb_frozen_hidden_states = model(data)
