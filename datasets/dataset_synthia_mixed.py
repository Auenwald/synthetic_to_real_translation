...
class SynthiaMixed(Dataset):
    def __init__(self, root_dir, split='train', transform=None):
        ...
        if self.split == 'train':
            self.images_orig = self.images_orig[0:8000]
            self.images_stylized = self.images_stylized[0:8000]
            self.masks = self.masks[0:8000]
        elif self.split == 'val':
            self.images_orig = self.images_orig[8000:8000+700]
            self.images_stylized = self.images_stylized[8000:8000+700]
            self.masks = self.masks[8000:8000+700]
        else:
            self.images_orig = self.images_orig[8000+700:]
            self.images_stylized = self.images_stylized[8000+700:]
            self.masks = self.masks[8000+700:]

    def __getitem__(self, index):

        if random.random() < 0.5:
            img = Image.open(self.images_orig[index]).convert('RGB')
        else:
            img = Image.open(self.images_stylized[index]).convert('RGB')
